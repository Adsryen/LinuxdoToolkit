...
- [ ] **代码分析与拆分** — 将单文件拆为多个职责模块
  - [ ] `button.js` — 悬浮入口 + 长按饼图菜单
  - [ ] `drawer.js` — iframe 抽屉模式
  - [ ] `preview.js` — 话题摘要预览
  - [ ] `auto-scroll.js` — 自动阅读滚动
  - [ ] `favorites.js` — 收藏夹 + 稍后读
  - [ ] `cache.js` — 话题缓存管理
  - [ ] `page-nav.js` — 页面导航记录
- [ ] **迁移存储** — GM_* → chrome.storage
- [ ] **接入工具栏** — 注册状态条：`预览就绪`
- [ ] **接入 Options 页** — 抽屉尺寸、自动滚动速度、缓存策略等

### Phase 6 — 界面美化模块（UI Enhance）

- [ ] **暗黑模式增强** — 基于 theme.js 的自动切换 + 手动覆盖
- [ ] **布局优化** — 可选的页面布局调整
- [ ] **自定义主题** — CSS 变量系统，支持自定义颜色方案
- [ ] **接入 Options 页** — 主题选择、自定义 CSS 编辑器

### Phase 7 — 打磨与发布

- [ ] **性能优化** — 懒加载模块、减少内容脚本体积
- [ ] **冲突检测** — 检测并提示与其他扩展/脚本的冲突
- [ ] **国际化基础** — 预留 i18n 结构
- [ ] **Chrome Web Store 上架**
- [ ] **文档完善** — 用户指南 + 开发者文档
- [ ] **CI/CD** — 自动构建 + 发布流程
